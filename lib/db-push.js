...
async function main() {
  ...
  // Only seed if no users exist yet (first-run only)
...
main().then(() => addContractsTable()).then(() => addUserRateColumns()).then(() => addWebsitesTable()).then(() => addQuickLinksTable()).then(() => addVaultTable()).catch(console.error)
